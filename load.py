"""Artemis Scanner Tracker v0.3.4 by Balvald."""
...
def journal_entry(cmdr: str, is_beta: bool, system: str, station: str, entry, state) -> None:  # noqa: CCR001
    ...
    if entry["event"] == "Resurrect":
        ...
        plugin.notyetsoldexplo[cmdr] = eventhandling.explo_resurrection_event()
    ...
    if entry["event"] == "Scan":
        if entry["ScanType"] == "AutoScan":
            eventhandling.explo_autoscan_event(entry, cmdr, plugin)

        if entry["ScanType"] == "Detailed":
            eventhandling.explo_detailedscan_event(entry, cmdr, plugin)

    if entry["event"] == "SAAScanComplete":
        eventhandling.explo_saascancomplete_event(entry, cmdr, plugin)
    ...
    if entry["event"] == "SellExplorationData":
        eventhandling.explo_sellexplorationdata_event(entry, cmdr, plugin)
    ...
    if entry["event"] == "MultiSellExplorationData":
        eventhandling.explo_multisellexplorationdata_event(entry, cmdr, plugin)
